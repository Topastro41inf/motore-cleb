...
import { Plus, RotateCcw, SquarePlus, UserPlus, Moon, Sun } from 'lucide-react';
...
function buildRows() {
  return Array.from({ length: MAX_ROWS }).map((_, i) => {
    const tpl = i < BASE_CYCLE.length ? BASE_CYCLE[i] : REPEATING_CYCLE[(i - BASE_CYCLE.length) % REPEATING_CYCLE.length];
    return {
      rowId: `row_${i + 1}`,
      rowIndex: i,
      code: tpl.code,
...
function placeQuota(state, quota) {
  ...
  if (!slot) throw new Error(`Nessuna cella disponibile per quota ${quota.kind}`);
...
function applyPallino(state, quota, triggerRow) {
  ...
  next.payouts.push({ payoutId: makeId('PAY'), quotaId: quota.id, ownerId: quota.ownerId, amount: 30 });
...
function addSquares(state, count) {
  ...
  for (let i = 0; i < n; i += 1) {
    cur = placeQuota(cur, createQuota({ kind: 'quadrato', ownerId: 'CLEB', visibilityType: 'sistema', systemGenerated: true }));
  }
  return runAuto(cur);
}

function addUser(state, label) {
  const next = clone(state);
  const code = makeUserCode();
  next.users.push({ id: code, label: label?.trim() || 'Nuovo socio' });
...
function createInitial() {
  ...
  for (let i = 0; i < START_TRIANGLES; i += 1) {
    s = placeQuota(s, createQuota({ kind: 'triangolo', ownerId: 'CLEB', visibilityType: 'sistema', systemGenerated: true }));
  }
...
function cellVisual(slot, quota, selectedOwnerId) {
  ...
  if (quota.kind === 'pallino') {
    return {
      symbol: '●',
      boxClass: isSelectedOwner ? 'border-emerald-400 bg-emerald-50 text-emerald-900 dark:border-emerald-500 dark:bg-emerald-950/40 dark:text-emerald-300' : 'border-slate-300 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100',
    };
  }
  if (quota.kind === 'triangolo') {
    return { symbol: '▲', boxClass: 'border-violet-300 bg-violet-50 text-violet-900 dark:border-violet-700 dark:bg-violet-950/40 dark:text-violet-300' };
  }
  return { symbol: '■', boxClass: 'border-blue-300 bg-blue-50 text-blue-900 dark:border-blue-700 dark:bg-blue-950/40 dark:text-blue-300' };
}

export default function MotoreCleb() {
  const [system, setSystem] = useState(createInitial());
  ...
  const [newUser, setNewUser] = useState({ nome: '', cognome: '', telefono: '', email: '', note: '' });
  ...
  const handleAddUser = () => {
    const label = `${newUser.nome} ${newUser.cognome}`.trim() || 'Nuovo socio';
    const next = addUser(system, label);
    const created = next.users[next.users.length - 1];
    if (created) {
      created.nome = newUser.nome;
      created.cognome = newUser.cognome;
      created.telefono = newUser.telefono;
      created.email = newUser.email;
      created.note = newUser.note;
    }
    setSystem({ ...next, users: [...next.users] });
    setSelectedOwnerId(created.id);
    setNewUser({ nome: '', cognome: '', telefono: '', email: '', note: '' });
  };

  const resetAll = () => setSystem(createInitial());
  ...
  return (
    <div className={darkMode ? "dark min-h-screen bg-slate-950 text-slate-100 p-6 relative overflow-hidden" : "min-h-screen bg-slate-50 text-slate-900 p-6 relative overflow-hidden"}>
      ...
      <div className={darkMode ? "pointer-events-none absolute inset-0 overflow-hidden opacity-100" : "pointer-events-none absolute inset-0 opacity-30"}>
        ...
        {darkMode && (
          <>
            <div className="absolute inset-x-0 top-0 h-72 bg-[radial-gradient(circle_at_20%_30%,rgba(56,189,248,0.14),transparent_40%),radial-gradient(circle_at_80%_20%,rgba(59,130,246,0.10),transparent_35%)]" />
            <div className="absolute top-8 left-[-8%] h-56 w-[116%] rounded-[100%] border-t border-cyan-400/20 blur-[1px]" />
            <div className="absolute top-16 left-[-6%] h-52 w-[112%] rounded-[100%] border-t border-sky-400/20" />
            <div className="absolute top-24 left-[-10%] h-48 w-[120%] rounded-[100%] border-t border-blue-400/15" />
            <div className="absolute top-40 left-[8%] h-44 w-[84%] rounded-[100%] border-t border-cyan-300/10" />
            <div className="absolute top-[28rem] left-[10%] h-40 w-[80%] rounded-[100%] border-t border-sky-500/10" />
            <div className="absolute top-[36rem] right-[-5%] h-48 w-[70%] rounded-[100%] border-t border-cyan-400/10" />
          </>
        )}
      </div>
      <div className="mx-auto max-w-7xl space-y-6 relative z-10">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" className={darkMode ? "rounded-2xl border-slate-700 bg-slate-900 text-slate-100 hover:bg-slate-800" : "rounded-2xl"} onClick={resetAll}>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          <Card className={darkMode ? "rounded-2xl border-slate-800 bg-slate-900/90" : "rounded-2xl"}><CardContent className="p-4"><div className="text-sm text-slate-500">Pallini</div><div className="text-2xl font-semibold">{summary.pallini}</div></CardContent></Card>
          <Card className={darkMode ? "rounded-2xl border-slate-800 bg-slate-900/90" : "rounded-2xl"}><CardContent className="p-4"><div className="text-sm text-slate-500">Triangoli</div><div className="text-2xl font-semibold">{summary.triangoli}</div></CardContent></Card>
          <Card className={darkMode ? "rounded-2xl border-slate-800 bg-slate-900/90" : "rounded-2xl"}><CardContent className="p-4"><div className="text-sm text-slate-500">Quadrati</div><div className="text-2xl font-semibold">{summary.quadrati}</div></CardContent></Card>
          <Card className={darkMode ? "rounded-2xl border-slate-800 bg-slate-900/90" : "rounded-2xl"}><CardContent className="p-4"><div className="text-sm text-slate-500">Pallini pagati</div><div className="text-2xl font-semibold">{summary.pagati}</div></CardContent></Card>
          <Card className={darkMode ? "rounded-2xl border-slate-800 bg-slate-900/90" : "rounded-2xl"}><CardContent className="p-4"><div className="text-sm text-slate-500">Soci</div><div className="text-2xl font-semibold">{summary.soci}</div></CardContent></Card>
          <Card className={darkMode ? "rounded-2xl border-slate-800 bg-slate-900/90" : "rounded-2xl"}><CardContent className="p-4"><div className="text-sm text-slate-500">Pagamenti creati</div><div className="text-2xl font-semibold">{summary.pagamenti}</div></CardContent></Card>
        </div>

        <div className="space-y-6">
          <Card className={darkMode ? "rounded-2xl shadow-sm border-slate-800 bg-slate-900/90" : "rounded-2xl shadow-sm"}>
            <CardHeader><CardTitle>Controlli rapidi</CardTitle></CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button className="rounded-2xl flex-1" onClick={handleAddQuota}>
                    <Plus className="mr-2 h-4 w-4" /> Aggiungi quota
                  </Button>
                  <Button className="rounded-2xl" variant="outline" onClick={() => setShowAdmin(!showAdmin)}>
                    <UserPlus className="mr-2 h-4 w-4" /> Gestione soci
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <div className="flex gap-2">
                  <Input type="number" min="1" value={squareCount} onChange={(e) => setSquareCount(e.target.value)} />
                  <Button className="rounded-2xl" variant="outline" onClick={handleAddSquares}>
                    <SquarePlus className="mr-2 h-4 w-4" /> Inserisci quadrati
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedUserQuotas.slice().reverse().slice(0, 8).map((q) => (
                    <Badge key={q.id} className={badgeClass(q.state)}>{q.id} · {q.visibilityType}</Badge>
                  ))}
                  {selectedUserQuotas.length === 0 && <div className="text-sm text-slate-500">Nessun pallino per questo socio.</div>}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Tabs defaultValue="albero" className="space-y-4">
            <TabsList className={darkMode ? "grid w-full grid-cols-3 rounded-2xl bg-slate-900/90 border border-slate-800" : "grid w-full grid-cols-3 rounded-2xl"}>
              <TabsTrigger value="albero">Albero</TabsTrigger>
            ...
            <TabsContent value="albero">
              <Card className={darkMode ? "rounded-2xl shadow-sm border-slate-800 bg-slate-900/90" : "rounded-2xl shadow-sm"}>
                <CardHeader><CardTitle>Struttura ciclica unica</CardTitle></CardHeader>
                    ...
                    {system.rows.map((row) => (
                      <div key={row.rowId} className="space-y-2 rounded-2xl border bg-white p-4">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <Badge className="bg-slate-100 text-slate-800">{row.slots.length} celle</Badge>
                        </div>
                          ...
                          {row.slots.map((slot) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {quota ? <Badge className={`mt-1 ${badgeClass(quota.state)}`}>{quota.state}</Badge> : <div className="mt-1 text-[10px]">vuota</div>}
                              </button>
                            );
                          })}
                        </div>
            ...
            <TabsContent value="quote">
              <Card className={darkMode ? "rounded-2xl shadow-sm border-slate-800 bg-slate-900/90" : "rounded-2xl shadow-sm"}>
                <CardHeader><CardTitle>Registro quote</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  {system.quotas.length === 0 && <div className="text-sm text-slate-500">Nessuna quota inserita.</div>}
                      ...
                      <div key={quota.id} className="rounded-2xl border bg-white p-4">
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                          <div>
                            ...
                            <div className="text-sm text-slate-600">{quota.kind} · {quota.visibilityType} · {owner?.label || quota.ownerId}</div>
                            <div className="text-sm text-slate-500">{quota.ownerId} · {quota.cellId}</div>
                          </div>
                          <Badge className={badgeClass(quota.state)}>{quota.state}</Badge>
                        </div>
                      </div>
                    );
                  })}
            ...
            <TabsContent value="eventi">
              <Card className={darkMode ? "rounded-2xl shadow-sm border-slate-800 bg-slate-900/90" : "rounded-2xl shadow-sm"}>
                <CardHeader><CardTitle>Log del motore</CardTitle></CardHeader>
                <CardContent className="space-y-2">
                  {system.events.length === 0 && <div className="text-sm text-slate-500">Ancora nessun evento.</div>}
          ...
          {showAdmin && (
            <Card className={darkMode ? "rounded-2xl shadow-sm border-slate-800 bg-slate-900/90" : "rounded-2xl shadow-sm"}>
              <CardHeader><CardTitle>Gestione soci</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label>Nome</Label>
                    <Input value={newUser.nome} onChange={(e) => setNewUser((p) => ({ ...p, nome: e.target.value }))} placeholder="Nome" />
                  </div>
                  <div className="space-y-2">
                    <Label>Cognome</Label>
                    <Input value={newUser.cognome} onChange={(e) => setNewUser((p) => ({ ...p, cognome: e.target.value }))} placeholder="Cognome" />
                  </div>
                  <div className="space-y-2">
                    <Label>Telefono</Label>
                    <Input value={newUser.telefono} onChange={(e) => setNewUser((p) => ({ ...p, telefono: e.target.value }))} placeholder="Telefono" />
                  </div>
                  <div className="space-y-2">
                    <Label>Email</Label>
                ...
                <div className="space-y-2">
                  <Label>Note</Label>
                  <Input value={newUser.note} onChange={(e) => setNewUser((p) => ({ ...p, note: e.target.value }))} placeholder="Note" />
                </div>

                <div className="flex justify-end">
                  <Button className="rounded-2xl" onClick={handleAddUser}>
                    <UserPlus className="mr-2 h-4 w-4" /> Crea socio
                  </Button>
                </div>
